document.addEventListener('DOMContentLoaded', function () {
    ...
    function createNodeElement(nodeDef, container, x, y) {
        // Prevent EVENT nodes in function graphs (except Function Start)
        if (nodeDef.type === 'EVENT' && window.currentGraphType === 'function' && !nodeDef.isFunctionStart) {
            console.warn('EVENT nodes are not allowed in function graphs');
            return null;
        }
        ...
        // Mark dynamic function nodes
        if (nodeDef.isFunctionStart || nodeDef.isReturn || nodeDef.isCustomFunction) {
            nodeEl.dataset.isDynamicNode = 'true';
            nodeEl.dataset.nodeDef = JSON.stringify(nodeDef);
        }
        ...
        } else if ((nodeDef.type === "METHOD" || nodeDef.type === "EVENT") && !(Array.isArray(nodeDef.exec)) && !nodeDef.noExecOutput) {
            const execOutputsContainer = document.createElement('div');
            execOutputsContainer.style.display = 'flex';
    ...
    // Helper function to create a node list item
    function createNodeListItem(nodeDef) {
        ...
        nodeItem.innerHTML = `
            <div style="display:flex;justify-content:space-between;align-items:center;padding:8px">
                <div>
                    <p class="node-name" style="font-weight:500;margin-bottom:4px">${nodeDef.title || nodeDef.name}</p>
        ...
        nodeItem.addEventListener('click', () => {
            ...
            createNodeElement(nodeDef, editorContent, x, y);
            closeSearchModal();
        });
    ...
    function populateNodesList() {
        ...
        // Add Function Start and Return nodes if in function graph mode
        if (window.currentGraphType === 'function' && window.generateFunctionStartNode && window.generateReturnNode) {
            const catEl = document.createElement('div');
            catEl.className = 'node-category';
            catEl.textContent = 'Function Nodes';
            nodesList.appendChild(catEl);

            // Add Function Start node
            const functionStartDef = window.generateFunctionStartNode();
            if (functionStartDef) {
                const nodeItem = createNodeListItem(functionStartDef);
                nodesList.appendChild(nodeItem);
            }

            // Add Return node
            const returnDef = window.generateReturnNode();
            if (returnDef) {
                const nodeItem = createNodeListItem(returnDef);
                nodesList.appendChild(nodeItem);
            }
        }

        for (const [category, nodes] of Object.entries(nodeDefinitions)) {
            // Filter out EVENT nodes if we're in function graph mode
            const filteredNodes = window.currentGraphType === 'function'
        ...
        // Add custom function call nodes (only in main graphs)
        if (window.currentGraphType === 'main' && window.customFunctions && window.customFunctions.length > 0) {
            const catEl = document.createElement('div');
            catEl.className = 'node-category';
            catEl.textContent = 'Custom Functions';
            nodesList.appendChild(catEl);
            ...
            window.customFunctions.forEach(func => {
                ...
                nodeItem.innerHTML = `
                    <div style="display:flex;justify-content:space-between;align-items:center;padding:8px">
                        <div>
                            <p class="node-name" style="font-weight:500;margin-bottom:4px">${nodeDef.title}</p>
                            <p style="font-size:12px;color:#a0a0a0">${nodeDef.description || ''}</p>
                        </div>
                        <span style="
                            background: rgba(${nodeDef.color.join(',')},0.2);
